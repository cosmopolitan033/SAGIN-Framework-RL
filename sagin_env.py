...
class SAGINEnv:
    def __init__(self, X, Y, duration, cache_size, compute_power_uav, compute_power_sat, compute_power_gv, energy,
                 max_queue, num_sats, num_iot_per_region, max_active_iot, ofdm_slots, num_vehicles=10):
        self.X, self.Y = X, Y
        ...
        # Air tier - UAVs
        self.uavs = {
            (x, y): UAV(x, y, X, Y, cache_size, max_queue, duration, compute_power_uav, energy, num_iot_per_region)
    ...
    def generate_and_offload_tasks(self):
        ...
        for (x, y), uav in self.uavs.items():
            ...
            for task in tasks:
                ...
                self.task_log.append(
                    {"task_id": task['task_id'], "source": f"uav_{x}_{y}", "assigned_to": offload_target})
        ...
        for v_id, vehicle in self.vehicles.items():
            ...
            for task in tasks:
                ...
                self.task_log.append(
                    {"task_id": task['task_id'], "source": f"vehicle_{v_id}", "assigned_to": offload_target})
        ...
        for (x, y), uav in self.uavs.items():
            ...
            for sat in self.sats:
                ...
                for content in uav.aggregated_content.values():
                    ...
                    sat_content['received_by_satellite'] = content[
                                                               'received_by_uav'] + transmission_delay + propagation_delay
                    print(
                        f"trans delay:{transmission_delay}, propagration: {propagation_delay}, receive: {sat_content['received_by_satellite']}")
            ...
            if (uav_x, uav_y) in self.connected_uavs:
                for sat in self.sats:
                    if ((uav_x, uav_y) in self.satellite_slots.get(sat.sat_id, []) and
                        ...
                        self.task_stats['satellite'][sat.sat_id]['received'] += 1
                        print(
                            f"[TASK {task_id}] Content {cid} found in SATELLITE {sat.sat_id} — offloaded via V2U-satellite")
                        return f'satellite_{sat.sat_id}'
    ...
    def step(self, actions=None, timestep=None):
        ...
        for v_id, vehicle in self.vehicles.items():
            ...
            if vehicle.energy <= 0:
                print(
                    f"[WARNING] Vehicle {v_id} has drained all energy at {self.g_timestep}. Removing from simulation.")
